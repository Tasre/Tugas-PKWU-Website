...
import { useAllTakedowns, useResolveTakedown, useListingTakedownMessages, useSendListingTakedownMessage } from "@/hooks/use-staff";
...
const TakedownManagement = () => {
  const { data: takedowns, isLoading } = useAllTakedowns();
  const resolve = useResolveTakedown();
  const [activeChat, setActiveChat] = useState<string | null>(null);

  if (isLoading) return <div className="text-muted-foreground text-sm flex items-center gap-2"><Clock className="w-4 h-4 animate-spin" /> Loading listing flags...</div>;

  if (!takedowns?.length) {
    return (
      <div className="text-center py-16 text-muted-foreground">
        <AlertCircle className="w-12 h-12 mx-auto mb-4 opacity-20" />
        <p className="font-display uppercase tracking-widest text-[10px] font-black opacity-40">No pending listing flags</p>
      </div>
    );
  }
          ...
          <div className="flex items-start justify-between">
            <div>
              <h4 className="font-display font-black text-foreground text-base leading-tight mb-1">
                {td.listings?.title || "Unknown Listing"}
              </h4>
              <p className="text-[10px] text-muted-foreground uppercase tracking-widest font-bold opacity-60">
                {td.listings?.game} • Seller: <span className="text-primary">{td.listings?.profiles?.username || "Unknown"}</span>
              </p>
            </div>
              ...
              {(td.status === "pending" || td.status === "seller_responded") && (
                <div className="flex gap-2">
                  <Button
                    size="sm"
                    variant="outline"
                    onClick={() => resolve.mutate({ id: td.id, listingId: td.listing_id, action: "cancel" })}
                  ...
                  <Button
                    size="sm"
                    variant="destructive"
                    onClick={() => resolve.mutate({ id: td.id, listingId: td.listing_id, action: "confirm" })}
                    disabled={resolve.isPending}
...
export default TakedownManagement;
