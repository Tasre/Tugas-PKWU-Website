...
BEGIN
    ...
    IF _required_role = 'admin' THEN RETURN user_role IN ('owner', 'admin'); END IF;
    IF _required_role = 'staff' THEN RETURN user_role IN ('owner', 'admin', 'staff'); END IF;
...
CREATE OR REPLACE FUNCTION "public"."notify_followers_on_new_post"() RETURNS "trigger"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
DECLARE
  ...
  follower RECORD;
BEGIN
  SELECT COALESCE(username, 'Someone') INTO author_name FROM public.profiles WHERE id = NEW.author_id;
  
  FOR follower IN SELECT follower_id FROM public.user_follows WHERE following_id = NEW.author_id
  LOOP
    ...
    VALUES (
      ...
      author_name || ' posted: "' || LEFT(NEW.title, 50) || '"',
      '/news'
    );
  END LOOP;
  RETURN NEW;
...
ALTER FUNCTION "public"."notify_followers_on_new_post"() OWNER TO "postgres";
...
CREATE OR REPLACE TRIGGER "on_new_post_notify_followers" AFTER INSERT ON "public"."posts" FOR EACH ROW EXECUTE FUNCTION "public"."notify_followers_on_new_post"();
...
CREATE OR REPLACE TRIGGER "on_post_dislike_sync" AFTER INSERT OR DELETE ON "public"."post_dislikes" FOR EACH ROW EXECUTE FUNCTION "public"."sync_post_dislikes_count"();



CREATE OR REPLACE TRIGGER "on_post_like_change" AFTER INSERT OR DELETE ON "public"."post_likes" FOR EACH ROW EXECUTE FUNCTION "public"."update_post_likes_count"();



CREATE OR REPLACE TRIGGER "on_post_like_sync" AFTER INSERT OR DELETE ON "public"."post_likes" FOR EACH ROW EXECUTE FUNCTION "public"."sync_post_likes_count"();
...
CREATE POLICY "Posts visibility" ON "public"."posts" FOR SELECT USING ((("status" = 'public'::"public"."post_status") OR ("auth"."uid"() = "author_id") OR (EXISTS ( SELECT 1
...
CREATE POLICY "Staffs can create posts" ON "public"."posts" FOR INSERT WITH CHECK ("public"."has_role"("auth"."uid"(), 'user'::"public"."app_role"));
...
CREATE POLICY "Users can manage their own subscriptions" ON "public"."user_subscriptions" WITH CHECK (("auth"."uid"() = "follower_id"));
...
GRANT ALL ON FUNCTION "public"."notify_followers_on_new_post"() TO "anon";
GRANT ALL ON FUNCTION "public"."notify_followers_on_new_post"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."notify_followers_on_new_post"() TO "service_role";
